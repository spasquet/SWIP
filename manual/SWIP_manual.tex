...
SWIP is a MATLAB-based software originally developped to work under a GNU/Linux distribution. It executes binaries from the open software packages \textbf{Seismic Unix} (available at \url{http://www.cwp.mines.edu/cwpcodes}) and \textbf{Geopsy} (available at \url{http://www.geopsy.org}). It (optionally) requires the open software packages \textbf{ImageMagick} (available at \url{http://www.imagemagick.org}), \textbf{PDFjam} (available at \url{http://www2.warwick.ac.uk/fac/sci/statistics/staff/academic-research/firth/software/pdfjam}), and \textbf{pdfcrop} (available at \url{http://www.ctan.org/pkg/pdfcrop}).

\bigskip\noindent
To work under Windows, it also requires to install the Linux-like environment \textbf{Cygwin} (available at \url{http://www.cygwin.com}) in order to run \textbf{Seismic Unix} binaries.
