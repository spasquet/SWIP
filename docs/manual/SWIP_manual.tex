...
\date{\LARGE{Version 1.2: September 2016}\\[2ex]
...
{\SWIP} has been successfully installed on Windows 7 and Windows 10. The procedure should be very similar on other Windows versions. As mentionned earlier, the Linux-like environment Cygwin is currently required to run {\SeismicUnix} on Windows. However, Microsoft has just announced that Windows 10 will soon be able to run Bash and compile Linux binaries with the Windows Subsystem for Linux (\url{https://msdn.microsoft.com/en-us/commandline/wsl/about}). Hopefully we will soon be able to run {\SeismicUnix} without any third party software like Cygwin.

\subsubsection{Cygwin}
\textit{The installation of Cygwin is only necessary to run {\SeismicUnix} (see \ref{subsec:WinSU}). and {\ImageMagick} (see \ref{subsec:WinExtra}). If you don't need {\SU} or {\ImageMagick}, you can skip the installation of Cygwin.}
...
Keep the default install, and add the following packages (use the search bar to find them, then click once on the circle arrow to select them):
...
Before starting, you need to create a project directory (referred later as \verb|projdir|) in which seismic data will be processed with {\SWIP}. This directory must contain the 5 module launchers of {\SWIP} located in \verb|/your/MATLAB/path/SWIP/launchers|.
...
\setlength\itemsep{2ex}
\item Regroup all your SEG2 files (e.g. \verb|.dat|, \verb|.sg2|,...) in a single folder (referred as \verb|rawdatadir|) located in your \verb|projdir| directory. SEG2 files should be named with increasing numbers (e.g. 1001.dat, 1002.dat,...). If your field data are in contained in a single SEGY file, just add this file in the folder instead of the SEG2 files. If you have topography along the line, add a two-column ASCII file (referred as \verb|topofile|) containing the distance and the elevation (X,Z).
\item In {\MATLAB}, go in \verb|projdir| and execute the command \verb|seg2su| in the command window.
\item Select the folder \verb|rawdatadir|.
\item To import topography, click \textit{Yes} when asked, then select the \verb|topofile| in \verb|rawdatadir|. You can add an altitude shift (in meters), and set up the scaling factor. The scaling factor is used to convert your X and Z coordinates in full integers since {\SU} can not read decimals. The factor has to be set according the desired coordinates precision (e.g. a scaling factor of 100 is required if you have a precision of 0.01~m, converting coordinates in centimeters).
...
\item When importing SEG2 or SEGY files with non-integer coordinates, only the integer part is read (e.g. 2.5~m becomes 2~m), so you might have to setup manually the geophones and shots coordinates. If that is the case, click \textit{No, reset all headers} to correctly define the coordinates in the headers. First enter the number of roll-alongs/deployments in the acquisition, then for each roll a vector [x1 x2 ... xn] of coordinates for the sources and the geophones. If you did not import topography (i.e. flat profile), you need to enter a scaling factor (see previous item).
\end{itemize}

\subsubsection{Import dispersion curves}
\label{sec:import}
...
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
\item \verb|nWmin| and \verb|nWmax| define the acceptable range of window sizes (in number of traces) for dispersion computation.
...
\item \verb|fcutlow| and \verb|fcuthigh| define the frequency limits of the filter along with the apodisation parameter \verb|taper|.

\item \verb|taper| defines the apodization parameter of the filter.
...
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
\item Set \verb|pick = 1| to manually pick dispersion curves. I recommend to first start the script with \verb|calc = 1| and \verb|pick = 0|, then once it's done computing dispersion images, restart the script with \verb|calc = 0| and \verb|pick = 1| (picking while calculating can take a really long time). To use the automatic picking option (\verb|pick = 2|), you need first to pick one dispersion curve with \verb|pick = 1|. It will then look for dispersion maxima in the range of the first picked dispersion curve.
...
\item \verb|dvmin| defines the minimum phase velocity sample (in m/s) used to display and pick the dispersion. This option is used to speed up display when picking, but will reduce the resolution of the picked phase velocity dispersion curves, which can be critical when dealing with small velocity variations. It has no impact on the resolution of saved dispersion images.

\item \verb|modeinit| defines the first mode that is picked (0 corresponding to the fundamental mode, 1 to the first higher mode,...). While picking, you will be able to change the picked mode for a specific $Xmid$, but it will always start with \verb|modeinit| for each new $Xmid$.
...
\item Set \verb|smoothpick = 1| to smooth the picks with a moving average filter, or \verb|smoothpick = 0| for no smoothing. While picking, you will be able to change the smoothing style for a specific $Xmid$, but it will always start with \verb|smoothpick| for each new $Xmid$.
\end{itemize}
...
\item \verb|D|: Delete one or several points
...
!! By default, this cutoff frequency is displayed during picking, but is not used to limit the range of the dispersion curve (\verb|freqlim = 0|). !!

\item \verb|fminpick| is the minimum frequency (in Hz) used to resample dispersion curves when \verb|freqlim = 0|. When \verb|freqlim = 1|, \verb|fminpick| will be the minimum automatic cutoff frequency allowed.
...
!! If you want to change the \verb|specampmin| threshold, you have to re-run the script with \verb|calc = 1|. !!
\end{itemize}

Target files are stored in \verb|subprojdir/file.targ| and contain all modes selected for inversion (cf option \verb|maxmodeinv|). They are named according to their $Xmid$ position (\verb|Xmid.target|). If a target file already exists for the selected $Xmid$ when using \verb|target = 1|, it will be overwritten. Run the script with \verb|calc = 0| and \verb|pick = 0| to change only the targets parameter (sampling, number of modes, error type,...).
...
\item Set \verb|plotpckdisp = 1| to plot and save stacked dispersion images with picked dispersion curves in \verb|subprojdir/file.img/1D\_data/disp\_pick| (\verb|plotpckdisp = 0| for no plot).
...
\paragraph{General display settings}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
\item \verb|imgform| defines the output format of figures. It can be PDF, PNG, JPEG, TIFF or FIG.
...
\paragraph{Dispersion images and curves settings}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
\item Set \verb|Dlogscale = 1| to use a "logarithmic" colorscale in the saved dispersion images (\verb|Dlogscale = 0| for linear colorscale).
...
\item \verb|pickcol1| and \verb|pickcol2| define the color of even and odd mode numbers, respectively, plotted on dispersion images when \verb|plotpckdisp = 1| ('w' for white, 'r' for red, 'k' for black, 'b' for blue ...).
...
The second module \verb|B_SWIPparam.m| allows to build a parameterization used in the inversion. It can either create a completely used defined parameterization, or a semi-automatic parameterization based on P-wave traveltime tomography results.
...
For \verb|paramtype > 0|, a specific \verb|Xmid.param| file will be stored for each $Xmid$ along with the \verb|Xmid.target| files in the \verb|subprojdir/file.targ| directory. The input of a three column ($X,Z,V_P$) ASCII file is required.
\end{itemize}
...
\item \verb|thmax| defines the maximum thickness of each layer (in m).
...
Set \verb|shape = 5| force a power law increase with depth.
...
\item \verb|Numax| defines the maximum Poisson's ratio of each layer.
...
\paragraph{Semi-automatic parameterization settings (used if paramtype = 1, 2, 3 or 4)}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
...
\item \verb|vfac| controls the range of $V_P$ ($Vpmin-vfac*Vpmin<v<Vpmax+vfac*Vpmax$).

\item Set \verb|linklvz = 1| to link $V_S$ with low velocity zones of $V_P$ (or \verb|linklvz = 0| for no link).
\end{itemize}
...
\item Set \verb|inversion = 1| to run the inversion (or \verb|inversion = 0| to use existing inversion results).

\item Set \verb|calcmod = 1| to extract and build average models from inversion results (or \verb|calcmod = 0| for no calculation, for instance to only plot inversion results or run the inversion).
\end{itemize}
...
\item Set \verb|verbose = 1| to display NA inversion process information (\verb|verbose = 0| for no display). Setting \verb|verbose = 0| considerably speed up the inversion process. If \verb|verbose = 0| and the inversion display never leaves "Run 1", restart the inversion with \verb|verbose = 1| to figure out the problem.
\end{itemize}
...
Inversion results are stored in \verb|subprojdir/file.inv/paramname_or_type/Xmid_reports| in gzipped files \verb|run_##.report.gz| along with the corresponding target file \verb|Xmid.target| (this prevents from losing the inverted data when modifying picks and/or sampling of targets in module A).

\paragraph{Average models calculation settings (used if calcmod=1)}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
...
The final models are stored, along with the standard deviation of the selected models, in \verb|subprojdir/file.inv/paramname_or_type/Xmid_reports|. All filenames are build as follow:
\verb|Xmid.extracttype.averagetype.modeltype|

Final models built with \verb|nbest = 0| use the following \verb|extracttype|: \verb|bweb| followed by the value of \verb|outpoints|. Final models built with \verb|nbest > 0| use the following \verb|extracttype|: \verb|best| followed by the value of \verb|nbest|.

Final models built with an arithmetic mean, along with the best and ridge models, use the following \verb|averagetype|: \verb|Vms|. Final models built with an misfit-weighted mean use the following \verb|averagetype|: \verb|Vws|. Standard deviations use the following \verb|averagetype|: \verb|VmsStd|.

\verb|modeltype| can be either \verb|best|, \verb|layered|, \verb|smooth| or \verb|ridge|.
...
\verb|subprojdir/file.img/paramname_or_type/models.extracttype/1dmodels/mod1d_Xmid| 

\item Set \verb|plot2dVS = 1| to display the 2D $V_S$ section during the inversion (\verb|plot2dVS = 0| for no display). This option is usefull to watch the 2D model being built, but it also slows down the inversion.
...
\item Set \verb|concat = 1| to concatenate figures and build a panel ready for publication (\verb|concat = 0| for no concatenation).\\[1ex]
!! {\ImageMagick} is required to concatenate rasters, while {\PDFjam} is required to concatenate PDF. !!

\item \verb|colnb| defines the number of columns in the panel figures.
...
\item \verb|map3| defines the colormap for the misfit of rejected models.
\end{itemize}

\paragraph{Calculated dispersion curves display settings (used if plotinvres=1)}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
...
\item \verb|VphMIN|, \verb|VphMAX| and \verb|Vphticks| control the display of phase velocity limits and ticks on inversion result plots (in m/s).
\end{itemize}

\paragraph{Calculated models display settings (used if plotinvres=1)}
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
...
\item \verb|dpMIN|, \verb|dpMAX| and \verb|dticks| control the display of depth limits and ticks on inversion result plots (in Hz).
...
\begin{itemize}[leftmargin=*]
\setlength\itemsep{2ex}
\item \verb|param1| defines the parameter plotted on the X axis. \verb|param1| can be 'Vs' for $V_S$, 'Th' for thickness, 'Vp' for $V_P$ and 'Dens' for density.

\item \verb|param1| defines the parameter plotted on the Y axis. \verb|param2| can be 'Vs' for $V_S$, 'Th' for thickness, 'Vp' for $V_P$ and 'Dens' for density.
...
The fourth module \verb|D1_SWIPmod1d.m| allows to plot final 1D models for each $Xmid$ and calculate the corresponding theoretical dispersion curves from: (i) {\SWIP} inversion results, (ii) combined P- and SH-wave refraction tomography results, and (iii) arbitrary velocity models.
...
\item Set \verb|tomo = 1| to use P- and SH-wave refraction tomography results extracted at each $Xmid$ to plot 1D models and compute theoretical dispersion, \verb|tomo = 0| otherwise.
...
!! Module \verb|C_SWIPinv.m| must have been run with \verb|calcmod = 1| and the same \verb|nbest| and \verb|outpoint| parameters, otherwise you will get a warning and nothing will be plotted. !!

\item Set \verb|usevptomo = 1| to extract $V_P$ at the corresponding $Xmid$ from a P-wave tomography model and use it to compute theoretical dispersion along with $V_S$ obtained from the {\SWIP} inversion (usefull for Poisson's ratio calculation). The input of a three column ($X,Z,V_P$) ASCII file is required. Set \verb|usevptomo = 0| to use the $V_P$ obtained from the {\SWIP} inversion (which cannot be trusted due to the weak influence of $V_P$ on the surface-wave dispersion).
...
\verb|subprojdir/file.img/paramname_or_type/models.extracttype/1dmodels/mod1d_Xmid|\\[1ex]
...
\item Set \verb|plot1vp = 1| to plot a 1D model of $V_P$ on a single graph with $V_S$ (\verb|plot1vp = 0| otherwise).
...
\item Set \verb|swip = 1| to use {\SWIP} inversion results to plot 2D models and compute theoretical dispersion, or \verb|swip = 2| to use P- (and SH-) wave tomography results.
\end{itemize}
...
\item Set \verb|plothisto = 1| to plot and save histogram of phase velocity residuals (\verb|plothisto = 0| for no plot).

\item Set \verb|plot2dmod = 1| to plot and save velocity models (\verb|plot2dmod = 0| for no plot).

\item Set \verb|plot2dert = 1| to plot and save resistivity model with a logarithmic colorscale (\verb|plot2dert = 0| for no plot). The input of a three column ($X,Z,\rho$) ASCII file is required.\\[1ex]
If \verb|swip = 1|, images are stored in:\\
\verb|subprojdir/file.img/paramname_or_type/models.extracttype/2dmodels|\\[1ex]
If \verb|swip = 2|, images are stored in:\\
\verb|subprojdir/file.img/Usermodels/2dmodels|

\item Set \verb|showplot = 1| to display the plots on the screen (\verb|showplot = 0| for no display).

\item Set \verb|savexzv = 1| to save 2D models in three column ($X,Z,V$) ASCII files (\verb|showplot = 0| for no save). These \verb|.xzv| files are stored in \verb|subprojdir/file.xzv/paramname_or_type/models.extracttype|
\end{itemize}
...
\item \verb|map7| defines the colormap for electrical resistivity 2D models.
...
\item \verb|resMIN|, \verb|resMAX| and \verb|resticks| control the display of electrical resistivity limits and ticks on 2D models.

\item \verb|resISO| defines the isocontours of electrical resistivity to plot over the 2D model
