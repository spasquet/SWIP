...
switch lower(location),
    case {'northeast'}
        set(legend_h, 'position', [pos(1)+fig_pos(3)-pos(3)-padding pos(2)+padding pos(3) pos(4)]);
    case {'northwest'}
        set(legend_h, 'position', [pos(1)+padding pos(2)+padding pos(3) pos(4)]);        
    case {'southeast'}
        pos(4) = newheight;
    ...
    case {'northoutside'}
        % need to resize axes to allow legend to fit in figure window
        shift = min([pos(4) fig_pos(4)]);
        set(gca, 'position', [fig_pos]-[0 -0.05*shift 0 0.3*shift]);
