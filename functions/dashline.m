...
try
    ...
    for i=1:sj
        ...
        distun=[0;cumsum(sqrt(diff(xposi).^2 + diff(yposi).^2))*10];
        [dist,I]=unique(distun);
        xdatai=xdataiun(I);
        ydatai=ydataiun(I);
        ...
        %Straight dashes ...
        if ~IsXLog
            xdash(1:length(dashes))=interp1(dist, xdatai, dashes,'linear');
        else
            xdash(1:length(dashes))=10.^interp1(dist, log10(xdatai), dashes,'linear');
        end 
        if ~IsYLog
            ydash(1:length(dashes))=interp1(dist, ydatai, dashes,'linear');
        else
            ydash(1:length(dashes))=10.^interp1(dist, log10(ydatai), dashes,'linear');
        end 
    ...
    set(gca,'Units',AxesUnits,'XLim',XLim,'YLim',YLim,'XLimmode',xlimmode,'YLimmode',ylimmode);
    if ~IsHold
        hold off
    end
catch
