...
stack       = 3;          % Standard stacking (=1), composite offset (=2, experimental) or WSSW (=3)

%%% Windowing and stacking settings (used if calc=1)
nWvec       = [3:4:33];   % Vector of window sizes (no. of traces)
dW          = 1;          % Shift between two successive windows (no. of traces)
dSmin       = 5;          % Min. offset between window and shot (no. of traces)
...
dvmin       = [];          % Min. phase velocity sampling (m/s)
...
min_resamp  = [];         % Resampling vector min. (wavelength [m] or frequency [Hz])
max_resamp  = [];         % Resampling vector max. (wavelength [m] or frequency [Hz])
n_resamp    = [];         % Number of samples in resampling vector
freqlim     = 0;          % Min. frequency defined with amplitude threshold (=1) or not (=0)
specampmin  = 0.005;       % Amplitude threshold (between 0 and 1) (used if freqlim=1)

%%% Uncertainty settings (used if target=1)
err         = 1;          % None (=0), Lorentz uncertainty (=1) or percentage uncertainty (=2)
minerrvel   = 20;         % Min. vel. uncertainty for Lorentz (m/s) (used if err=1)
maxerrrat   = 0.3;        % Max. vel. uncertainty ratio for Lorentz (used if err=1)
...
Dlogscale   = 1;          % Pseudo-log colorscale for dispersion (=1) or linear (=0)
...
vertex      = 0.4;                  % Vertical exageration
